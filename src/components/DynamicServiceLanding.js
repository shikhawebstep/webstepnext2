        ...
        <motion.div variants={stagger} initial="hidden" animate="show">
          ...
          <motion.div variants={fadeUp} className="mt-9 flex flex-col sm:flex-row gap-3 sm:gap-4">
              <Link href="/contactus" className="inline-flex items-center justify-center gap-2 sm:gap-3 rounded-full bg-gradient-to-r from-[#FF1F8E] via-[#A855F7] to-[#38BDF8] px-5 sm:px-7 py-3.5 sm:py-4 text-xs sm:text-sm font-bold sm:font-black uppercase tracking-wider sm:tracking-widest text-white shadow-[0_16px_36px_rgba(168,85,247,0.25)] transition hover:-translate-y-0.5">
                <span>{hero.primaryCta || (isHire ? "Hire Developers" : "Apply Now")}</span> <FiArrowRight className="shrink-0 text-base" />
              </Link>
              ...
              {pricing.map((plan) => (
                <motion.article key={plan.title} variants={fadeUp} className="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm">
                  <div className="flex flex-row items-start justify-between gap-4">
                    <div>
                      <h3 className="text-lg sm:text-xl font-black text-slate-950">{plan.title}</h3>
                      <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm leading-relaxed text-slate-600">{plan.description}</p>
                    </div>
                    <div className="shrink-0 self-start rounded-full bg-pink-50 border border-pink-100/80 px-3.5 py-1.5 text-xs sm:text-sm font-bold sm:font-black text-pink-600">{plan.price}</div>
                  </div>
                  <div className="mt-5 flex flex-wrap gap-2">
                    {(plan.items || []).map((item) => (
